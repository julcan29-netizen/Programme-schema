...
def draw_frame(dwg):
    ...
    text(dwg, 75, 95, "PUISSANCE", size=28, weight="bold")
    ...
    text(dwg, cx + 490, cy + 22, "FOLIO", size=11)
    text(dwg, cx + 512, cy + 47, "10", size=22, weight="bold")
...
def draw_buses(dwg):
    ...
    text(dwg, 40, Y_L + 5, "L", size=17, weight="bold")
    text(dwg, 40, Y_N + 5, "N", size=17, weight="bold")
    text(dwg, 30, Y_PE + 5, "PE", size=17, weight="bold")
...
def draw_a1(dwg):
    x, y = X_A1, Y_A1
    text(dwg, x + 55, y - 24, "A1", 15, "bold", "middle")
    text(dwg, x + 55, y - 8, "Régulateur MXPRO", 11, "middle")
    rect(dwg, x, y, 110, 86)
    text(dwg, x + 55, y + 52, "MXPRO", 18, "bold", "middle")
...
def draw_supply_drops(dwg):
    ...
    a1n = X_A1
    line(dwg, a1n, Y_N, a1n, Y_A1 + 60, 1.3)
    node(dwg, a1n, Y_N)
    wire_no(dwg, a1n + 8, Y_A1 + 38, "2")
    ...
    ps1n = X_PS1
    line(dwg, ps1n, Y_N, ps1n, Y_PS1 + 60, 1.3)
    node(dwg, ps1n, Y_N)
    wire_no(dwg, ps1n + 8, Y_PS1 + 38, "6")
...
def draw_wires(dwg):
    ...
    line(dwg, dm1_out[0], dm1_out[1], dm1_out[0], km1_in[1] - 38, 1.3)
    line(dwg, dm1_out[0], km1_in[1] - 38, km1_in[0], km1_in[1] - 38, 1.3)
    line(dwg, km1_in[0], km1_in[1] - 38, km1_in[0], km1_in[1], 1.3)
    ...
    s1 = (X_T1 + 18, Y_T1 + 108)
    s2 = (X_T1 + 70, Y_T1 + 108)
    line(dwg, s1[0], s1[1], s1[0], s1[1] + 80, 1.3)
    wire_no(dwg, s1[0] + 8, s1[1] + 42, "20")
    cross_ref(dwg, s1[0] + 18, s1[1] + 82, "→ 11 T1")

    line(dwg, s2[0], s2[1], s2[0], s2[1] + 110, 1.3)
    wire_no(dwg, s2[0] + 8, s2[1] + 56, "21")
    cross_ref(dwg, s2[0] + 18, s2[1] + 112, "→ 11 T1")

    kref_y = Y_KM1 + 18
    line(dwg, X_KM1 + 38, kref_y, X_KM1 + 150, kref_y, 1.3)
    wire_no(dwg, X_KM1 + 78, kref_y - 6, "17")
    cross_ref(dwg, X_KM1 + 160, kref_y, "→ 11 KM1")

    aoref_y = Y_YV1 + 24
    line(dwg, X_YV1, aoref_y, X_YV1 + 120, aoref_y, 1.3)
    wire_no(dwg, X_YV1 + 48, aoref_y - 6, "34")
    cross_ref(dwg, X_YV1 + 130, aoref_y, "→ 11 YV1 AO")
